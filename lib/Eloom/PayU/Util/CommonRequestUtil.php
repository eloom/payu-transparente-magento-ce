...
class Eloom_PayU_Util_CommonRequestUtil {
  ...
  protected static function buildCommonRequest($lang, $command) {
    ...
    $request->test = Eloom_PayU_PayU::$isTest;
    return $request;
  }
  ...
  protected static function buildMerchant() {
    ...
    $merchant->apiLogin = Eloom_PayU_PayU::$apiLogin;
    $merchant->apiKey = Eloom_PayU_PayU::$apiKey;
    return $merchant;
  }
  ...
  protected static function buildCreditCard($parameters) {
    ...
    $creditCard->expirationDate = self::getParameter($parameters, Eloom_PayU_Util_PayUParameters::CREDIT_CARD_EXPIRATION_DATE);
    $creditCard->securityCode = self::getParameter($parameters, Eloom_PayU_Util_PayUParameters::CREDIT_CARD_SECURITY_CODE);
    $creditCard->processWithoutCvv2 = (bool) self::getParameter($parameters, Eloom_PayU_Util_PayUParameters::PROCESS_WITHOUT_CVV2);
  ...
  protected static function isValidDate($dateString, $dateFormat, $parameterName) {
    if (DateTime::createFromFormat($dateFormat, $dateString) == FALSE) {
      ...
      sprintf("The [%s] format is invalid. Use [%s] ", $parameterName, $dateFormat));
    }
    return true;
  }
