...
class Eloom_PayU_Util_HttpClientUtil {
   ...
   */
  static function sendRequest($request, Eloom_PayU_Api_PayUHttpRequestInfo $payUHttpRequestInfo) {
    $httpHeader = array(
        self::CONTENT_TYPE,
        self::CONTENT_LENGTH . strlen($request),
        self::ACCEPT);
    if ((isset($payUHttpRequestInfo->lang))) {
      array_push($httpHeader, self::ACCEPT_LANGUAGE . '$payUHttpRequestInfo->lang');
    }
    //$logger = Eloom_Bootstrap_Logger::getLogger(__CLASS__);
    //$logger->info($request);
    ...
    if ($curlResponse === false && $httpStatus === 0) {
      throw new Eloom_PayU_Exceptions_ConnectionException(Eloom_PayU_Exceptions_PayUErrorCodes::CONNECTION_EXCEPTION, 'the url [' . $payUHttpRequestInfo->getUrl() . '] did not respond');
    }

    if ($curlResponse === false) {
      $requestInfo = http_build_query(curl_getinfo($curl), ' ', ',');
      $curlMsgError = sprintf(" error occured during curl exec info: curl message[%s], curl error code [%s], curl request details [%s]", curl_error($curl), curl_errno($curl), $requestInfo);
